...
f = open('semi_synthetic_loss.txt', 'w', buffering = 1)
...
for CHOICE1 in range(len(arr1)):
    for CHOICE2 in range(len(arr1)):
        for CHOICE3 in range(len(arr1)):
            ...
            stable_rand_cv, stable_block_cv, stable_prop_cv = run_cv(all_stable_X, all_y, ts, log_loss)
            unstable_rand_cv, unstable_block_cv, unstable_prop_cv = run_cv(all_unstable_X, all_y, ts, log_loss)
